...
export const TagListItem = (props: { name?: string; onClick: () => void }) => {
    ...
    return (
        <ListItem button className={classes.root} onClick={props.onClick} component={Link} to={`tags/${props.name}`}>
            <ListItemIcon>
                <PageviewTwoToneIcon />
            </ListItemIcon>
            <ListItemText primaryTypographyProps={{ variant: "overline" }} primary={props.name} />
        </ListItem>
    );
};
